...
def log(message):
    ...
    st.session_state.log_output += f"{timestamp}{message}\n"
...
# 🧪 Add a test message to confirm visibility
log("✅ Terminal initialized successfully.")

# -------------------- Terminal Output Panel --------------------
st.markdown("### 🖥 Terminal Output (Live Logs)")

# Add Clear Logs button
if st.button("🧹 Clear Terminal Logs"):
    st.session_state.log_output = ""

# Display the styled terminal window
st.markdown(
    f"""
    <div style="background-color:#000000; padding:10px; border-radius:10px; height:400px; overflow-y:auto;">
        <pre style="color:white; font-family:monospace; font-size:13px;">{st.session_state.log_output}</pre>
    </div>
    """,
    unsafe_allow_html=True,
)
